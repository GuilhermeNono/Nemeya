...
public readonly struct SqlQueryValidator
{
    ...
    public void BuildVirtualVariables(IFilter? filter)
    {
        if(filter is null)
            return;
    ...
    private void UpdateCollectionVariableToScalarParams(IFilter filter)
    {
        var properties = filter.GetType().GetProperties();
        
        foreach (var property in properties)
        {
            if (!IsACollection(property.GetValue(filter), out var collection))
                continue;
